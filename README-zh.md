...
- Antigravity 通过真实的 CDP 鼠标事件选中绑定的 CC 对话。
...
- **显式分组绑定**：每个分组拥有一个 CC 对话和一个 Codex 线程。
...
| 功能 | 作用 |
| --- | --- |
| 分组绑定 | 多个 CC/Codex 对保持独立：个人维基、开源项目、QA 等互不干扰。 |
| 绑定目标路由 | 命令使用分组保存的 CC 标题和 Codex 线程 ID，不依赖当前窗口。 |
...
| `node bridge.mjs bind-cc --title "标题"` | 把当前分组绑定到一个 Antigravity 对话。 |
...
| `node bridge.mjs ag-send --text "消息"` | 向绑定的 CC 对话发送文本。 |
| `node bridge.mjs ag-send --group "开源项目" --text "消息"` | 向指定分组的 CC 对话发送文本。 |
...
| `node bridge.mjs cc-to-codex` | 把最新的 CC 回复发送给 Codex。 |
| `node bridge.mjs codex-to-cc` | 把最新的 Codex 回复发送回 CC。 |
| `node bridge.mjs cc-to-codex --dry-run` | 预览 CC → Codex 但不发送。 |
| `node bridge.mjs codex-to-cc --dry-run` | 预览 Codex → CC 但不发送。 |
...
| `POST /api/bind/cc` | 把当前分组绑定到一个 CC 对话标题。 |
