...
export async function getAutoAcceptStatus(options = {}) {
  const [plugin, settings] = await Promise.all([
    findLatestAutoAcceptExtension(options.extensionsRoot),
    readAutoAcceptSettings(options.settingsPath),
  ]);
  const targets = await listCdpTargets(settings.cdpPort).catch(() => null);
  return {
    plugin,
    settings,
    cdp: {
...
export async function listAutoAcceptConversations(options = {}) {
  const settings = await readAutoAcceptSettings(options.settingsPath);
  const targets = await listCdpTargets(options.port ?? settings.cdpPort);
...
async function resolveTarget(options) {
  await findLatestAutoAcceptExtension(options.extensionsRoot);
  const settings = await readAutoAcceptSettings(options.settingsPath);
  const targets = await listCdpTargets(options.port ?? settings.cdpPort);
