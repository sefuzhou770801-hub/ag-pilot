...
async function route(request, response) {
  ...
  if (request.method === "POST" && url.pathname === "/api/open-codex") {
    const state = await loadState();
    if (!state.data.codexThreadId) return sendJson(response, 400, { ok: false, error: "Codex thread is not bound" });
    await openCodexThread(state.data.codexThreadId);
...
export async function snapshot() {
  ...
  // 如果有绑定的 Codex thread，查标题
  const boundThreadTitle = state.data.codexThreadId
    ? await getCodexThreadTitle(state.data.codexThreadId).catch(() => null)
    : null;
  return {
    ok: true,
    state,
