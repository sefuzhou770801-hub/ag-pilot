...
async function route(request, response) {
  ...
  if (request.method === "POST" && url.pathname === "/api/groups/switch") {
    ...
    if (!groupId) return sendJson(response, 400, { ok: false, error: "Group id is required" });
    const state = await loadState();
    const data = setActiveGroup(state, groupId);
    await saveState(data);
    return sendJson(response, 200, { ok: true, data });
  }
  if (request.method === "POST" && url.pathname === "/api/bind/cc") {
    ...
    const state = await loadState();
    const data = updateActiveGroup(state, { ccTitle: title });
    await saveState(data);
    return sendJson(response, 200, { ok: true, data });
  }
  if (request.method === "POST" && url.pathname === "/api/bind/codex") {
    ...
    const state = await loadState();
    const data = updateActiveGroup(state, { codexThreadId: threadId });
    await saveState(data);
    return sendJson(response, 200, { ok: true, data });
  }
  ...
  if (request.method === "POST" && url.pathname === "/api/open-codex") {
    ...
    if (!activeGroup?.codexThreadId) return sendJson(response, 400, { ok: false, error: "Codex thread is not bound" });
    await openCodexThread(activeGroup.codexThreadId);
...
export async function snapshot() {
  ...
  const boundThreadTitle = activeGroup?.codexThreadId
    ? await getCodexThreadTitle(activeGroup.codexThreadId).catch(() => null)
    : null;
  return {
    ok: true,
    state,
    ...
    codexDb,
    boundThreadTitle,
    view: buildCardViewModel({ state, status, conversations, codexThreads, codexDb, boundThreadTitle }),
  };
}
