...
async function route(request, response, dependencies) {
  ...
  if (request.method === "POST" && url.pathname === "/api/bind/ag") {
    const body = await readJson(request);
    const title = body.title;
    if (!title) return sendJson(response, 400, { ok: false, error: "Antigravity title is required" });
    const state = await dependencies.loadState();
    const data = dependencies.updateViewGroup(state, { agTitle: title });
    await dependencies.saveState(data);
    return sendJson(response, 200, { ok: true, data });
  }
...
async function selectBoundAntigravityConversation(group, dependencies = defaultDependencies) {
  if (!group?.agTitle) return { ok: true, skipped: true };
  try {
    const result = await dependencies.selectAntigravityConversation({ title: group.agTitle });
...
async function readAntigravityBusy(viewGroup, antigravity, dependencies) {
  if (!viewGroup?.agTitle || !antigravity?.cdp?.connected) {
