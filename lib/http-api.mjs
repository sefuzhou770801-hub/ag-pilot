...
async function route(request, response) {
  ...
  if (request.method === "POST" && url.pathname === "/api/groups/switch") {
    ...
    void selectBoundAntigravityConversation(viewGroup);
...
async function selectBoundAntigravityConversation(group) {
  if (!group?.ccTitle) return { ok: true, skipped: true };
  try {
    const result = await selectAntigravityConversation({ title: group.ccTitle });
...
export async function snapshot() {
  ...
  state = await autoClearStaleBusy(state);
  ...
  const conversations = await listAntigravityConversations().catch(() => []);
  ...
  const boundThreadTitle = viewGroup?.codexThreadId
    ...
    : null;
  const antigravityBusy = await readAntigravityBusy(viewGroup, antigravity);
  return {
    ok: true,
    state,
    ...
    codexDb,
    boundThreadTitle,
    view: buildCardViewModel({ state: viewState, status, conversations, codexThreads, codexDb, boundThreadTitle, antigravityBusy }),
  };
}

async function readAntigravityBusy(viewGroup, antigravity) {
  if (!viewGroup?.ccTitle || !antigravity?.cdp?.connected) {
    return { known: false, busy: null };
  }
  try {
    return { known: true, busy: await isAntigravityTargetBusy() };
  } catch (error) {
...
async function autoClearStaleBusy(state) {
  ...
  const updatedGroups = await Promise.all(
    state.data.groups.map(async (group) => {
      ...
      try {
        const rolloutPath = await findLatestCodexRollout(group.codexThreadId);
