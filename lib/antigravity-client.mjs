...
export async function readLatestAntigravityReply(options = {}) {
  ...
  try {
    await selectConversationIfNeeded(client, target);
    const text = await client.evaluate(latestAssistantTextScript());
    if (!text) throw new Error("No Antigravity assistant reply found in the selected Antigravity target");
    return { text, targetTitle: target.conversationTitle ?? target.title ?? "", targetId: target.id ?? "" };
  } finally {
    client.close();
  }
}
...
export async function sendTextToAntigravity(options = {}) {
  ...
  try {
    ...
    if (focusResult !== "ready") {
      throw new Error(`Antigravity input is not ready in target "${target.conversationTitle ?? target.title ?? target.id}"`);
    }
    await client.insertText(options.text);
    await client.pressEnter();
    return { sent: true, targetTitle: target.conversationTitle ?? target.title ?? "", targetId: target.id ?? "" };
  } finally {
    client.close();
  }
}

export async function selectAntigravityConversation(options = {}) {
  if (!options.title) throw new Error("Antigravity conversation title is required");
  const config = await readBridgeConfig(options);
  const targets = await listCdpTargets(options.port ?? config.cdpPort);
  const manager = findManagerTarget(targets);
  if (!manager?.webSocketDebuggerUrl) {
    throw new Error("No Antigravity Manager target found");
  }

  const client = new CdpClient(manager.webSocketDebuggerUrl);
  try {
    const result = await clickConversation(client, options.title);
    return { selected: true, title: result.title, targetId: manager.id ?? "" };
  } finally {
    client.close();
  }
}
...
export async function isAntigravityTargetBusy(options = {}) {
  const config = await readBridgeConfig(options);
  const targets = await listCdpTargets(options.port ?? config.cdpPort);
  const target = findBusyReadTarget(targets);
  if (!target?.webSocketDebuggerUrl) {
    throw new Error("No Antigravity target found for busy check");
  }
  const client = new CdpClient(target.webSocketDebuggerUrl);
  try {
...
export function findBusyReadTarget(targets) {
  return findCurrentChatTarget(targets);
}

export async function captureAntigravityScreenshot(options = {}) {
  const config = await readBridgeConfig(options);
  const targets = await listCdpTargets(options.port ?? config.cdpPort);
  const target = findCurrentChatTarget(targets) ?? findManagerTarget(targets);
  if (!target?.webSocketDebuggerUrl) throw new Error("No Antigravity target found for screenshot");
  const client = new CdpClient(target.webSocketDebuggerUrl);
  try {
    const frame = await client.send("Page.captureScreenshot", { format: "png" });
    const base64 = frame?.result?.data;
...
export async function stopAntigravityAgent(options = {}) {
  const config = await readBridgeConfig(options);
  const targets = await listCdpTargets(options.port ?? config.cdpPort);
  const target = findCurrentChatTarget(targets);
  if (!target?.webSocketDebuggerUrl) return { stopped: false, reason: "no target" };
  const client = new CdpClient(target.webSocketDebuggerUrl);
  try {
    const clicked = await client.evaluate(`(() => {
      const panel = document.querySelector('.antigravity-agent-side-panel') || document;
      const stopBtn = panel.querySelector('button[aria-label*="stop"], button[aria-label*="Stop"], [data-tooltip-id*="cancel"], [data-tooltip-id*="stop"]');
      if (stopBtn) { stopBtn.click(); return true; }
      return false;
    })()`);
    return { stopped: Boolean(clicked) };
  } finally {
    client.close();
  }
}
...
async function resolveTarget(options) {
  ...
  if (!target && options.title) {
    const manager = findManagerTarget(targets);
    if (manager) target = { ...manager, conversationTitle: options.title };
  }
  if (!target) {
...
export function findTargetByTitle(targets, title) {
  ...
  return candidateTargets(targets).find((target) => {
    ...
    const titlePrefix = hay.slice(0, 20);
    return hay.includes(needle) || (titlePrefix.length >= 8 && needle.includes(titlePrefix));
  }) ?? null;
}
...
async function selectConversationIfNeeded(client, target) {
  if (!target?.conversationTitle) return;
  await clickConversation(client, target.conversationTitle);
}

async function clickConversation(client, title) {
  const result = await client.evaluate(conversationClickPointScript(title));
  if (!result?.found) {
    throw new Error(`No Antigravity conversation found for title "${title}"`);
  }
  await client.clickAt(result.x, result.y);
  await sleep(200);
  return result;
}
...
export function conversationClickPointScript(title) {
  return `(() => {
    return new Promise((resolve) => {
      const wanted = ${JSON.stringify(String(title ?? ""))};
      const clean = (text) => (text || '').replace(/\\s+/g, ' ').trim();
      const normalize = (text) => clean(text).toLowerCase();
      ...
      const readPoint = (item, pill, title) => {
        const itemRect = item.getBoundingClientRect();
        const pillRect = pill.getBoundingClientRect();
        const rect = itemRect.width > 0 && itemRect.height > 0 ? itemRect : pillRect;
      ...
      for (const pill of pills) {
        const title = clean(pill.innerText || pill.textContent);
        if (!matches(title)) continue;
        const item = pill.closest('div[role="button"], div[class*="select-none"][class*="cursor-pointer"][class*="rounded-md"]') || pill;
        item.scrollIntoView({ block: 'center', inline: 'nearest' });
        requestAnimationFrame(() => readPoint(item, pill, title));
        return;
      }
