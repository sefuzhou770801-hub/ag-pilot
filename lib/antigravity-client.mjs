...
export async function selectAntigravityConversation(options = {}) {
  ...
  const client = new CdpClient(manager.webSocketDebuggerUrl);
  try {
    const result = await client.evaluate(selectConversationScript(options.title));
    if (!result?.found) {
      throw new Error(`No Antigravity conversation found for title "${options.title}"`);
    }
    await sleep(500);
...
async function selectConversationIfNeeded(client, target) {
  ...
  const result = await client.evaluate(selectConversationScript(target.conversationTitle));
  if (!result?.found) {
    throw new Error(`No Antigravity conversation found for title "${target.conversationTitle}"`);
  }
  await sleep(500);
}
...
function selectConversationScript(title) {
  return `(() => {
