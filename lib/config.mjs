...
export function normalizeStateData(raw = {}) {
  ...
  if (activeGroup && (Object.hasOwn(source, "agTitle") || Object.hasOwn(source, "ccTitle"))) {
    activeGroup.agTitle = String(source.agTitle ?? source.ccTitle ?? "");
  }
...
export function findGroupByCodexThreadId(stateOrData, threadId) {
  const data = normalizeStateData(unwrapStateData(stateOrData));
  const needle = String(threadId ?? "").trim();
  if (!needle) return null;
  return data.groups.find((group) => group.codexThreadId === needle) ?? null;
}
...
export function updateGroup(stateOrData, groupId, updates = {}) {
  const data = normalizeStateData(unwrapStateData(stateOrData));
  if (!data.groups.some((group) => group.id === groupId)) {
    throw new Error("Group not found");
  }
...
export function updateGroupsByCodexThreadId(stateOrData, threadId, updates = {}) {
  const data = normalizeStateData(unwrapStateData(stateOrData));
  const needle = String(threadId ?? "").trim();
  if (!needle) return { data, groups: [] };

  const groups = data.groups.map((group) => {
    if (group.codexThreadId !== needle) return group;
    return normalizeGroup({ ...group, ...updates }, 0, group.name);
  });
  const matchedGroups = groups.filter((group) => group.codexThreadId === needle);

  return {
    data: withActiveAliases({
      groups,
      activeGroupId: data.activeGroupId,
      viewGroupId: data.viewGroupId,
      updatedAt: new Date().toISOString(),
...
function normalizeGroup(group, index, fallbackName = defaultGroupName(index + 1)) {
  ...
  return {
    ...
    agTitle: String(group.agTitle ?? group.ccTitle ?? ""),
...
function toStoredStateData(data) {
  ...
  if (normalized.paused) stored.paused = true;
  return stored;
}
...
function needsStateMigration(raw) {
  ...
  if (raw.groups.some((group) => !Object.hasOwn(group ?? {}, "codexBusy"))) return true;
  if (raw.groups.some((group) => Object.hasOwn(group ?? {}, "ccTitle"))) return true;
  return Object.hasOwn(raw, "agTitle") || Object.hasOwn(raw, "codexThreadId");
}
